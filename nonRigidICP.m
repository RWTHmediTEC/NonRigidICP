function [transformed_vertices, weights, accumulated_transformations] = nonRigidICP(mesh_template, mesh_target, varargin)

% NONRIGIDICP   Non-rigid alignment of two meshes (e.g. from STL files)
%
%    NONRIGIDICP(mesh_template, mesh_target)
%    NONRIGIDICP(mesh_template, mesh_target, ...)
%
...
%
%    The positional input arguments 'mesh_template' and 'mesh_target' must
...
%
%       % Perform the fitting.
%       transformed_target.faces = template.faces;
%       transformed_target.vertices = nonRigidICP(template, target);
%
%
...
n = size(mesh_template.vertices, 1); % # template vertices
...
% Initialize accumulated transformations and mesh_template.vertices
if ~isequal(transformation_init, [eye(3), [0;0;0]])
    % Transform the mesh_template.vertices with the initial transformation matrix.
    template_vertices_homogeneous = [mesh_template.vertices, ones(n, 1)];
    transformed_vertices_homogeneous = template_vertices_homogeneous * transformation_init';
    mesh_template.vertices = transformed_vertices_homogeneous(:, 1:3);
    ...
    if sign(det(transformation_init(1:3,1:3))) == -1
        mesh_template.faces = [mesh_template.faces(:,1), mesh_template.faces(:,3), mesh_template.faces(:,2)];
    end
end
...
%myPlot(mesh_template.vertices, mesh_template.faces, mesh_target.vertices, mesh_target.faces, 0.5, 1.0); pause();

% Create structures for efficient point2planeDist calculation
kd_tree_target = KDTreeSearcher(mesh_target.vertices, 'distance', 'euclidean'); % vertex distance KD-Tree
v2f_target = buildVerticesToFacesList(mesh_target.vertices, mesh_target.faces); % vertex-face mapping
if isfield(mesh_target, 'normals')
    mesh_target.vertex_normals = STLVertexNormals(mesh_target.faces, mesh_target.vertices, mesh_target.normals);
else
    mesh_target.vertex_normals = STLVertexNormals(mesh_target.faces, mesh_target.vertices);% vertex normals
end

v2f_template = buildVerticesToFacesList(mesh_template.vertices, mesh_template.faces); % vertex-face mapping - needed to calculate (area) vertex weights
e = buildEdgeList(mesh_template.faces); % the cost function needs a template edge list
...
for a = alpha'
    ...
    for iter = 1:max_iter
        ...
        mesh_template.vertex_normals = STLVertexNormals(mesh_template.faces, mesh_template.vertices); % compute vertex normals (face area weighted)

        [U, weights, ~] = closestPointsOnSurface(...
            mesh_template, mesh_target, ...
        ...
        vWeight = weights.*computeVertexWeight(mesh_template, v2f_template);
        ...
        mesh_template.vertices = full(D*X);
